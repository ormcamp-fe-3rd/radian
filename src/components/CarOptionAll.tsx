...
interface CarOptionProps {
  carType: 'sport' | 'compact' | 'utility';
  onColorChange: (color: string) => void;
}

const CarOption: React.FC<CarOptionProps> = ({ carType, onColorChange }) => {
  ...
  const totalPrice =
  (selectedBattery ? BATTERY_PRICE_MAP[carType]?.[selectedBattery] || 0 : 0) +
  (selectedDrive ? DRIVE_PRICE_MAP[carType]?.[selectedDrive] || 0 : 0) +
  (selectedSound ? SOUND_PRICE_MAP[selectedSound] || 0 : 0);

if (!BATTERY_PRICE_MAP[carType] || !DRIVE_PRICE_MAP[carType]) {
  console.error(`Invalid carType or missing data for: ${carType}`);
...
<div className="option-grid">
  {Object.values(Battery).map((battery) => (
    ...
    >
      <span className="option-text">{BATTERY_LABEL_MAP[battery]}</span> {/* Mapping Object 사용 */}
      <span className="option-price">
        +{BATTERY_PRICE_MAP[carType][battery]?.toLocaleString() || '0'}원
      </span>
    </div>
  ))}
</div>
...
<div className="option-grid">
  {Object.values(Drive).map((drive) => (
    ...
    >
      <span className="option-text">{DRIVE_LABEL_MAP[drive]}</span> {/* Mapping Object 사용 */}
      <span className="option-price">
        +{DRIVE_PRICE_MAP[carType][drive]?.toLocaleString() || '0'}원
      </span>
    </div>
  ))}
</div>
...
<div className="option-grid">
  {Object.values(Sound).map((sound) => (
    ...
    >
      <span className="option-text">{SOUND_LABEL_MAP[sound]}</span> {/* Mapping Object 사용 */}
      <button
