...
const CarObicontrol: React.FC<CarObicontrolProps> = ({ color }) => {
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      undefined,
      (error) => {
        console.error('Error loading utility-paint.gltf:', error);
      },
    );

    // Load body.gltf (with texture)
    loader.load(
      '/images/ProductReservation/utility-body.gltf',
      (gltf) => {
        const model = gltf.scene;
      ...
      undefined,
      (error) => {
        console.error('Error loading utility-body.gltf:', error);
      },
    );

    // Load background.gltf (with texture)
    loader.load(
      '/images/ProductReservation/utility-floor.gltf',
      (gltf) => {
        const model = gltf.scene;
      ...
      undefined,
      (error) => {
        console.error('Error loading utility-floor.gltf:', error);
      },
    );
    ...
    loader.load(
      '/images/ProductReservation/utility-glass1.gltf',
      (gltf) => {
        const material = new THREE.MeshStandardMaterial({
      ...
      undefined,
      (error) => {
        console.error('Error loading utility-glass1.gltf.gltf:', error);
      },
    );

    // Load Glass Cover (유리 덮개)
    loader.load(
      '/images/ProductReservation/utility-glass2.gltf',
      (gltf) => {
        const material = new THREE.MeshStandardMaterial({
    ...
    // Load Lamp Bulb (전구)
    loader.load(
      '/images/ProductReservation/utility-lamp.gltf',
      (gltf) => {
