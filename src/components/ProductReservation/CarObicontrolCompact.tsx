...
const CarObicontrol: React.FC<CarObicontrolProps> = ({ color }) => {
  ...
  useEffect(() => {
    ...
    rgbeLoader.load(
      '/images/ProductReservation/compact-vr2.hdr',
    ...
    loader.load(
      ...
      undefined,
      (error) => {
        console.error('Error loading compact-paint.gltf:', error);
      },
    );

    // Load body.gltf (with texture)
    loader.load(
      '/images/ProductReservation/compact-body8.gltf',
      (gltf) => {
        const model = gltf.scene;
      ...
      undefined,
      (error) => {
        console.error('Error loading compact-body.gltf:', error);
      },
    );
    ...
    loader.load(
      '/images/ProductReservation/compact-glass1.gltf',
      (gltf) => {
        const material = new THREE.MeshStandardMaterial({
      ...
      undefined,
      (error) => {
        console.error('Error loading compact-glass1.gltf.gltf:', error);
      },
    );

    // Load Glass Cover (유리 덮개)
    loader.load(
      '/images/ProductReservation/compact-glass2.gltf',
      (gltf) => {
        const material = new THREE.MeshStandardMaterial({
    ...
    // Load Lamp Bulb (전구)
    loader.load(
      '/images/ProductReservation/compact-lamp.gltf',
      (gltf) => {
