...
const CarOption: React.FC<CarOptionProps> = ({ onColorChange }) => {
  ...
  const totalPrice =
    (selectedBattery ? BATTERY_PRICE_MAP['compact'][selectedBattery] : 0) +
    (selectedDrive ? DRIVE_PRICE_MAP['compact'][selectedDrive] : 0) +
        ...
        {Object.values(Battery).map((battery) => (
          ...
          >
            ...
            <span className="option-price">
              +{BATTERY_PRICE_MAP['compact'][battery]?.toLocaleString() || '0'}원
            </span>
          </div>
        ))}
      </div>
      ...
      <div className="option-grid">
        {Object.values(Drive).map((drive) => (
          ...
          >
            ...
            <span className="option-price">
              +{DRIVE_PRICE_MAP['compact'][drive]?.toLocaleString() || '0'}원
            </span>
          </div>
        ))}
      </div>
